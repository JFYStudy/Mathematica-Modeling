...
var SimplifiedDict = dict;

// src/traditional_dict.ts
var dict2 = {
...
var Fuzyy_chinese = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.api = {pinyin: Pinyin, p: PinyinKeys, q: Query};
  }
  async onload() {
...
var main_default = Fuzyy_chinese;
var FuzzyModal = class extends import_obsidian.SuggestModal {
  constructor(app2, plugin) {
    ...
    this.scope.register(["Mod"], "Enter", async (e) => {
      ...
      app2.workspace.getLeaf("tab").openFile(file);
    });
    this.scope.register(["Mod", "Alt"], "Enter", async (e) => {
      this.close();
      let file = await this.getChooseItemFile();
      app2.workspace.getLeaf("split").openFile(file);
    });
    this.scope.register(["Shift"], "Enter", async (e) => {
      if (this.inputEl.value == "")
        return;
      this.close();
      let nf = await app2.vault.create(app2.fileManager.getNewFileParent("").path + "/" + this.inputEl.value + ".md", "");
    ...
    this.scope.register(["Mod", "Shift"], "Enter", async (e) => {
      ...
      this.close();
      let nf = await app2.vault.create(app2.fileManager.getNewFileParent("").path + "/" + this.inputEl.value + ".md", "");
      app2.workspace.getLeaf("tab").openFile(nf);
    });
    this.scope.register(["Shift", "Alt"], "Enter", async (e) => {
      ...
      this.close();
      let nf = await app2.vault.create(app2.fileManager.getNewFileParent("").path + "/" + this.inputEl.value + ".md", "");
      getNewOrAdjacentLeaf(app2.workspace.getMostRecentLeaf()).openFile(nf);
    });
    this.scope.register(["Alt"], "Enter", async (e) => {
      this.close();
      let file = await this.getChooseItemFile();
      getNewOrAdjacentLeaf(app2.workspace.getMostRecentLeaf()).openFile(file);
    });
    if (app2.plugins.plugins["obsidian-hover-editor"])
  ...
  getSuggestions(query) {
    if (query == "") {
      ...
      let lastOpenFiles = app.workspace.getLastOpenFiles().map((p) => items.find((q) => q.type == "file" && q.path == p)).filter((p) => p).map((p) => {
        return {
          item: p,
          score: -1,
          range: null
    ...
    let indexNode = this.historyMatchData.index(index - 1), toMatchData = indexNode.itemIndex.length == 0 ? this.items : indexNode.itemIndex;
    ...
    if (this.plugin.settings.usePathToSearch && matchData1.length <= 10) {
      ...
      for (let p of toMatchData.filter((p2) => p2.type == "file" && !matchData1.map((p3) => p3.item.path).includes(p2.path))) {
  ...
  renderSuggestion(matchData, el) {
    el.addClass("fz-item");
    let range = matchData.range, score = matchData.score, text, e_content = el.createEl("div", {cls: "fz-suggestion-content"}), e_title = e_content.createEl("div", {cls: "fz-suggestion-title"});
    switch (score) {
      case -1: {
    ...
    if (this.plugin.settings.showTags) {
      let tags = app.metadataCache.getFileCache(matchData.item.file)?.frontmatter?.tags || app.metadataCache.getFileCache(matchData.item.file)?.frontmatter?.tag, tagArray;
      if (tags) {
    ...
    if (matchData.item.type == "alias") {
      ...
      e_flair.innerHTML += '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="svg-icon lucide-forward"><polyline points="15 17 20 12 15 7"></polyline><path d="M4 18v-2a4 4 0 0 1 4-4h12"></path></svg>';
  ...
  async onChooseSuggestion(matchData, evt) {
    let file = matchData.score == -2 ? await app.vault.create(app.fileManager.getNewFileParent("").path + "/" + this.inputEl.value + ".md", "") : matchData.item.file;
    if (evt.ctrlKey) {
  ...
  onNoSuggestion() {
    this.chooser.setSuggestions([{item: {type: "file"}, score: -2}]);
    this.chooser.addMessage(this.emptyStateText);
  }
  async getChooseItemFile() {
...
var FuzzySettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(app2, plugin) {
    super(app2, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("\u4F7F\u7528\u8DEF\u5F84\u641C\u7D22").setDesc("\u5F53\u641C\u7D22\u7ED3\u679C\u5C11\u4E8E10\u4E2A\u65F6\u641C\u7D22\u8DEF\u5F84").addToggle((text) => text.setValue(this.plugin.settings.usePathToSearch).onChange(async (value) => {
      this.plugin.settings.usePathToSearch = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("\u663E\u793A\u8DEF\u5F84").addToggle((text) => text.setValue(this.plugin.settings.showPath).onChange(async (value) => {
      this.plugin.settings.showPath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("\u663E\u793A Tag").addToggle((text) => text.setValue(this.plugin.settings.showTags).onChange(async (value) => {
      this.plugin.settings.showTags = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("\u7E41\u4F53\u652F\u6301").addToggle((text) => {
...
var Pinyin = class extends Array {
  constructor(query, usePath = false) {
    super();
    this.usePath = usePath;
    this.query = query.toLowerCase();
    this.query.split("").forEach((p) => {
      let index = PinyinValues.map((q, i) => q.includes(p) ? i : null).filter((p2) => p2);
      this.push({
  ...
  getScore(range) {
    let score = 0;
    score += 40 / (this.query.length - range.reduce((p, i) => p + i[1] - i[0] + 1, 0));
    if (range[0][0] == 0)
      score += 8;
    score += 20 / range.length;
    return score;
  }
  getRange(query) {
    let index = 0, list = [], currentStr = query[index].query;
    for (let i = 0; i < this.length; i++) {
      let el = this[i], type = query[index].type;
      switch (type) {
        case "pinyin": {
          if (el.type == "pinyin") {
            let f = el.pinyin.map((pinyin) => currentStr.startsWith(pinyin) || pinyin.startsWith(currentStr));
            if (f.some((p) => p)) {
              list.push(i);
              currentStr = currentStr.substring(el.pinyin[f.findIndex((p) => p)].length);
            } else if (el.pinyin.some((pinyin) => pinyin.startsWith(currentStr[0]))) {
              list.push(i);
...
var PinyinIndex = class extends import_obsidian.Component {
  constructor(app2, plugin) {
    super();
    this.plugin = plugin;
  ...
  initIndex() {
    let files, startTime = Date.now();
    if (this.plugin.settings.showAllFileTypes)
      files = app.vault.getFiles();
    else if (this.plugin.settings.showAttachments)
      files = app.vault.getFiles().filter((f) => extension.attachment.includes(f.extension));
    else
      files = app.vault.getFiles().filter((f) => extension.normal.includes(f.extension));
    ...
    console.log(`Fuzzy Chinese Pinyin: Indexing completed, totaling ${files.length} files, taking ${(Date.now() - startTime) / 1e3}s`);
  }
  initEvent() {
    this.registerEvent(this.metadataCache.on("changed", (file, data, cache) => {
  ...
  isEffectiveFile(file) {
    ...
    if (this.plugin.settings.showAllFileTypes)
      return true;
    else if (this.plugin.settings.showAttachments && extension.attachment.includes(file.extension))
      return true;
    else if (extension.normal.includes(file.extension))
      return true;
    else
      return false;
  }
};
function TFile2Item(file) {
  let name = file.extension != "md" ? file.name : file.basename;
  return {type: "file", file, name, pinyin: new Pinyin(name), path: file.path, pinyinOfPath: new Pinyin(file.path, true)};
}
function CachedMetadata2Item(file, cache) {
